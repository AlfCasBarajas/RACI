                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-body">
                        <h2 class="card-title text-success">
                            <i class="bi bi-folder2-open me-2"></i>Reporte de Categorías
                        </h2>
                        ...
                        <form method="GET" action="" class="row g-3 align-items-center mb-3">
                            ...
                            <div class="col-auto">
                                <input type="text" id="nombre" name="nombre" class="form-control" 
                                       placeholder="Nombre de la categoría" value="<?= htmlspecialchars($nombre ?? '') ?>">
                            </div>
                            <div class="col-auto">
                                <button type="submit" class="btn btn-primary">Filtrar</button>
                            </div>
                            <div class="col-auto">
                                <a href="?controller=reportes&action=categorias" class="btn btn-secondary">Limpiar filtros</a>
                            </div>
                        </form>
                                    ...
                                    <?php if (!empty($categorias)): ?>
                                        <?php foreach ($categorias as $categoria): ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($categoria['area_nombre'] ?? 'Sin área') ?></td>
                                                <td><?= htmlspecialchars($categoria['empleado_nombre'] ?? 'Sin empleado') ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="5" class="text-center">No hay categorías para mostrar</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
...
document.getElementById('descargar-pdf').onclick = function(e) {
    ...
    let url = '?controller=reportes&action=categorias&format=pdf';
...
document.getElementById('descargar-excel').onclick = function(e) {
    e.preventDefault();
    const nombre = document.getElementById('nombre').value;
    let url = '?controller=reportes&action=categorias&format=excel';
    if (nombre) url += '&nombre=' + encodeURIComponent(nombre);
    window.location.href = url;
};
</script>
