            ...
            <div class="container mt-4">
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-body">
                        ...
                        <form method="GET" action="" class="row g-3 align-items-center mb-3">
                            ...
                            <div class="col-auto">
                                <input type="text" id="nombre" name="nombre" class="form-control" 
                                       placeholder="Nombre del área" value="<?= htmlspecialchars($nombre ?? '') ?>">
                            </div>
                            <div class="col-auto">
                                <button type="submit" class="btn btn-primary">Filtrar</button>
                            </div>
                            <div class="col-auto">
                                <a href="?controller=reportes&action=areas" class="btn btn-secondary">Limpiar filtros</a>
                            </div>
                        </form>
                        ...
                        <!-- Tabla de áreas -->
                        <div id="tabla-areas" class="table-responsive">
                                    ...
                                    <?php if (!empty($areas)): ?>
                                        <?php foreach ($areas as $area): ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($area['descripcion'] ?? 'Sin descripción') ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="3" class="text-center">No hay áreas para mostrar</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
...
document.getElementById('descargar-pdf').onclick = function(e) {
    ...
    let url = '?controller=reportes&action=areas&format=pdf';
...
document.getElementById('descargar-excel').onclick = function(e) {
    e.preventDefault();
    const nombre = document.getElementById('nombre').value;
    let url = '?controller=reportes&action=areas&format=excel';
    if (nombre) url += '&nombre=' + encodeURIComponent(nombre);
    window.location.href = url;
};
</script>
